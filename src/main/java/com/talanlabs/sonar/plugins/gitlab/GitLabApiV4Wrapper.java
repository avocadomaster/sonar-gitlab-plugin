...
public class GitLabApiV4Wrapper implements IGitLabApiWrapper {
    ...
    public void createOrUpdateSonarQubeStatus(String status, String statusDescription) {
        try {
            gitLabAPIV4.getGitLabAPICommits().postCommitStatus(gitLabProject.getId(), getFirstCommitSHA(), status, config.refName(), COMMIT_CONTEXT, config.targetUrl(), statusDescription);
        } catch (IOException e) {
